...
tr_rand_buffer (void   * buffer,
                size_t   length)
{
  ctr_drbg_context rng_state;

  assert (buffer != NULL);

  if (!check_result (ctr_drbg_init (&rng_state, &my_rand, NULL, NULL, 0)))
    return false;

  return check_result (ctr_drbg_random (&rng_state, buffer, length));
}
                       ...
                       size_t     * output_length)
{
  ...
  if ((err = base64_encode (NULL, &ret_length, input, input_length)) != 0)
    if (err != POLARSSL_ERR_BASE64_BUFFER_TOO_SMALL)
      {
        log_error (err);
        return NULL;
      }
  ...
  if (!check_result (base64_encode ((unsigned char *) ret, &ret_length, input, input_length)))
    {
      tr_free (ret);
      return NULL;
    }
                       ...
                       size_t     * output_length)
{
  ...
  if ((err = base64_decode (NULL, &ret_length, input, input_length)) != 0)
    if (err != POLARSSL_ERR_BASE64_BUFFER_TOO_SMALL)
      {
        log_error (err);
        return NULL;
  ...
  if (!check_result (base64_decode ((unsigned char *) ret, &ret_length, input, input_length)))
    {
      tr_free (ret);
      return NULL;
    }
